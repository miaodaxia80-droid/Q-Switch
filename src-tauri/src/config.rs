...
pub fn get_home_dir() -> PathBuf {
    if let Ok(home) = std::env::var("QSWITCH_TEST_HOME") {
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    let default_dir = get_home_dir().join(".qswitch");
    ...
    #[cfg(windows)]
    {
        let default_db = default_dir.join("qswitch.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".qswitch");
                    if legacy_dir.join("qswitch.db").exists() {
                        log::info!(
