...
impl McpService {
    ...
    pub fn upsert_server(state: &AppState, server: McpServer) -> Result<(), AppError> {
        ...
        if prev_apps.qoder && !server.apps.qoder {
            Self::remove_server_from_app(state, &server.id, &AppType::Qoder)?;
        }
    ...
    fn sync_server_to_app_no_config(server: &McpServer, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::Qoder => {
                // The ACP runtime loads Qoder-enabled servers directly from
                // the database at prompt time. Keeping this a no-op preserves
                // Qoder installation integrity and makes enablement immediate.
                log::debug!("Qoder MCP server enabled for the local ACP runtime");
            }
        }
        Ok(())
    }
    ...
    fn remove_server_from_app(_state: &AppState, id: &str, app: &AppType) -> Result<(), AppError> {
        match app {
            ...
            AppType::Qoder => {
                log::debug!("Qoder MCP server disabled for the local ACP runtime");
            }
        }
        Ok(())
    }
