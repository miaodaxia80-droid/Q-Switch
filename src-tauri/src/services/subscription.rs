...
/// Gemini OAuth client credentials are supplied by the local runtime environment,
/// never embedded in the source tree or release artifacts.
fn gemini_oauth_client_credentials() -> Option<(String, String)> {
    let client_id = std::env::var("QSWITCH_GEMINI_OAUTH_CLIENT_ID").ok()?;
    let client_secret = std::env::var("QSWITCH_GEMINI_OAUTH_CLIENT_SECRET").ok()?;
