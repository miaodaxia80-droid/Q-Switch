...
pub(crate) fn remove_common_config_from_settings(
    ...
    match app_type {
        ...
        | AppType::Hermes
        | AppType::ClaudeDesktop
        | AppType::Qoder => Ok(settings.clone()),
    }
}
...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        ...
        AppType::Qoder => {
            log::debug!(
                "Qoder provider '{}' stored in CC Switch; ACP Agent reads the database SSOT",
                provider.id
            );
        }
    }
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        ...
        AppType::Qoder => Err(AppError::localized(
            "qoder.live.read_unsupported",
            "Qoder 供应商由 CC Switch 数据库和本地 ACP Agent 管理。",
            "Qoder providers are managed by the CC Switch database and local ACP Agent.",
        )),
    }
}
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        ...
        AppType::Qoder => {
            return Err(AppError::localized(
                "qoder.import_unsupported",
                "Qoder 没有可导入的 live 供应商配置，请在 CC Switch 中新增。",
                "Qoder has no importable live provider config. Add it in CC Switch.",
            ));
        }
    };
