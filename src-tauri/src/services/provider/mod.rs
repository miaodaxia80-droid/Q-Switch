...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("QSWITCH_TEST_HOME").ok();

            env::set_var("HOME", dir.path());
            #[cfg(windows)]
            env::set_var("LOCALAPPDATA", dir.path().join("AppData").join("Local"));
            env::set_var("USERPROFILE", dir.path());
            env::set_var("QSWITCH_TEST_HOME", dir.path());
    ...
    impl Drop for TempHome {
        fn drop(&mut self) {
            ...
            match &self.original_test_home {
                Some(value) => env::set_var("QSWITCH_TEST_HOME", value),
                None => env::remove_var("QSWITCH_TEST_HOME"),
            }
        }
    }
    ...
    fn with_test_home<T>(test: impl FnOnce(&AppState, &Path) -> T) -> T {
        ...
        let old_test_home = std::env::var_os("QSWITCH_TEST_HOME");
        let old_home = std::env::var_os("HOME");
        std::env::set_var("QSWITCH_TEST_HOME", temp.path());
        ...
        match old_test_home {
            Some(value) => std::env::set_var("QSWITCH_TEST_HOME", value),
            None => std::env::remove_var("QSWITCH_TEST_HOME"),
        }
        match old_home {
