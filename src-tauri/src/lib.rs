...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("qswitch://") {
        return false;
    }
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/cc-switch.log）
            {
                ...
                log::info!("=== Qswitch v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            // Qswitch Qoder 测试包不注册自动更新器。它必须保持与 CC Switch
            // 完全独立，不能读取或安装 CC Switch 的发布通道。
            ...
            let db_path = app_config_dir.join("qswitch.db");
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) =
                    crate::qoder_config::sync_qoder_model_manifest(&qoder_manifest_db).await
                {
                    log::warn!("Failed to refresh Qoder model manifest at startup: {error}");
                }
            });
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/qswitch-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first qswitch:// URL
                        }
                    }
                }
            });
            ...
            tauri::async_runtime::spawn(async move {
                ...
                // Native Qoder routes through the local HTTP proxy without
                // taking over Qoder's own Live configuration.
                match state.db.get_all_providers("qoder") {
                    Ok(providers) if !providers.is_empty() => {
                        if let Err(error) = state.proxy_service.start().await {
                            log::warn!("Failed to start local proxy for Qoder: {error}");
                        }
                    }
                    Ok(_) => {}
                    Err(error) => {
                        log::warn!("Failed to inspect Qoder providers at startup: {error}");
                    }
                }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Qoder local routing catalog
            commands::qoder_sync_model_manifest,
            commands::qoder_list_routes,
            commands::qoder_list_carrier_mappings,
            commands::qoder_save_carrier_mapping,
            commands::qoder_remove_carrier_mapping,
            commands::qoder_get_tool_policy,
            commands::qoder_set_tool_policy,
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older Qswitch version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including qswitch.db)\n\
            2) If you see \u{201C}database version is newer\u{201D}, please upgrade Qswitch\n\
