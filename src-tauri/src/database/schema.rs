...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute(
            ...
            enabled_qoder BOOLEAN NOT NULL DEFAULT 0
        )",
            [],
        )
        .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        // 8. Proxy Config 表（按应用独立配置，app_type 主键）
        conn.execute("CREATE TABLE IF NOT EXISTS proxy_config (
            app_type TEXT PRIMARY KEY CHECK (app_type IN ('claude','codex','gemini','grokbuild','qoder')),
        ...
        if Self::has_column(conn, "proxy_config", "app_type")? {
            ...
            conn.execute(
                ...
                circuit_failure_threshold, circuit_success_threshold, circuit_timeout_seconds,
                circuit_error_rate_threshold, circuit_min_requests)
                VALUES ('qoder', 3, 60, 120, 600, 4, 2, 60, 0.6, 10)",
                [],
            )
            .map_err(|e| AppError::Database(e.to_string()))?;
        }
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（添加 Qoder 代理配置）");
                        Self::migrate_v16_to_v17(conn)?;
                        Self::set_user_version(conn, 17)?;
                    }
                    17 => {
                        log::info!("迁移数据库从 v17 到 v18（添加 Qoder MCP 启用状态）");
    ...
    /// v16 -> v17：重建 proxy_config 以允许 Qoder 独立使用代理配置。
    ///
    /// SQLite 无法直接修改 CHECK 约束，因此保留现有值后重建表。
    fn migrate_v16_to_v17(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!("v16 -> v17 创建 proxy_config 失败: {e}")))?;
        ...
        .map_err(|e| AppError::Database(format!("v16 -> v17 复制 proxy_config 失败: {e}")))?;

        conn.execute("DROP TABLE proxy_config", [])
            .map_err(|e| AppError::Database(format!("v16 -> v17 删除旧 proxy_config 失败: {e}")))?;
        conn.execute("ALTER TABLE proxy_config_v17 RENAME TO proxy_config", [])
            .map_err(|e| AppError::Database(format!("v16 -> v17 重命名 proxy_config 失败: {e}")))?;
        conn.execute(
        ...
        .map_err(|e| AppError::Database(format!("v16 -> v17 初始化 Qoder 配置失败: {e}")))?;

        Ok(())
    }

    /// v17 -> v18: Qoder's local ACP adapter reads enabled MCP server
    /// definitions directly from this database. No Qoder application bundle
    /// files are modified.
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_adds_qoder_proxy_row_and_check_constraint() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        Database::create_tables_on_conn(&conn)?;
        conn.execute("DELETE FROM proxy_config WHERE app_type = 'qoder'", [])?;
        conn.execute(
            "UPDATE proxy_config SET enabled = 1, max_retries = 11 WHERE app_type = 'codex'",
            [],
        )?;
        ...
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        let qoder_values: (i64, i64) = conn.query_row(
            "SELECT enabled, max_retries FROM proxy_config WHERE app_type = 'qoder'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        assert_eq!(qoder_values, (0, 3));
        ...
        assert_eq!(codex_values, (1, 11));

        let rejected = conn.execute(
            "INSERT INTO proxy_config (app_type) VALUES ('unsupported-app')",
            [],
        );
        assert!(
            rejected.is_err(),
            "proxy_config CHECK should reject unknown apps"
        );
    ...
    #[test]
    fn migrate_v17_to_v18_adds_qoder_mcp_flag() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            "CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
        ...
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        assert!(Database::has_column(&conn, "mcp_servers", "enabled_qoder")?);
        let values: (i64, i64) = conn.query_row(
            "SELECT enabled_codex, enabled_qoder FROM mcp_servers WHERE id = 'mcp-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
