...
impl RequestContext {
    ...
    pub async fn new(
        ...
        Self::new_with_provider_id(state, body, headers, app_type, tag, app_type_str, None).await
    }

    /// Create a request context pinned to a specific provider.
    ///
    /// Qoder custom-model routes carry an opaque route ID in the `model`
    /// field. The local Q Switch gateway resolves that ID to both the real
    /// upstream model and provider. Pinning here keeps the route deterministic
    /// without mutating Q Switch's globally selected provider.
    pub async fn new_with_provider_id(
        ...
        let current_provider_id = preferred_provider_id
            .map(str::to_string)
            .unwrap_or_else(|| {
        ...
        let providers = if let Some(provider_id) = preferred_provider_id {
            let provider = state
                .db
                ...
                .ok_or_else(|| {
                    ProxyError::InvalidRequest(format!(
                        "Qoder route provider is unavailable: {provider_id}"
                    ))
                })?;
            vec![provider]
        } else {
