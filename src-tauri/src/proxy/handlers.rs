...
use super::{
    ...
    handler_config::{
        ...
        CODEX_PARSER_CONFIG, GEMINI_PARSER_CONFIG, OPENAI_PARSER_CONFIG, QODER_PARSER_CONFIG,
    },
...
/// GET /qoder/v1/models — non-secret Q Switch model routes for Qoder.
///
/// The patched Qoder renderer polls this loopback endpoint so provider/model
/// changes become visible without embedding credentials or repeatedly
/// rewriting the Qoder bundle.
pub async fn handle_qoder_models(
    State(state): State<ProxyState>,
) -> Result<axum::response::Response, ProxyError> {
    let path = crate::qoder_config::sync_qoder_model_manifest(&state.db)
        .await
        .map_err(|error| ProxyError::Internal(error.to_string()))?;
    let bytes = std::fs::read(&path).map_err(|error| {
        ProxyError::Internal(format!("Failed to read Qoder model manifest: {error}"))
    })?;
    let value: Value = serde_json::from_slice(&bytes).map_err(|error| {
        ProxyError::Internal(format!("Failed to parse Qoder model manifest: {error}"))
    })?;
    Ok((
        [
...
/// 处理 Qoder 的 OpenAI Chat Completions 请求。
///
/// Qoder 原生 Agent 负责 SSE、工具调用和文件编辑；这里仅提供独立的
/// Provider/故障转移/用量归因命名空间，避免与 Codex 共用配置。
pub async fn handle_qoder_chat_completions(
    State(state): State<ProxyState>,
    request: axum::extract::Request,
) -> Result<axum::response::Response, ProxyError> {
    handle_chat_completions_for_app(state, request, AppType::Qoder, "Qoder", "qoder").await
}
...
async fn handle_chat_completions_for_app(
    ...
    let qoder_route = if matches!(app_type, AppType::Qoder) {
        let requested_model = body
            .get("model")
            .and_then(Value::as_str)
            .unwrap_or_default()
            .to_string();
        let route = crate::qoder_config::resolve_qoder_route(&requested_model)
            .map_err(|error| ProxyError::Internal(error.to_string()))?;
        if route.is_none()
            && requested_model
                .strip_prefix("custom:")
        ...
        {
            return Err(ProxyError::InvalidRequest(format!(
                "Unknown Q Switch model route: {requested_model}"
            )));
        }
        if let Some(route) = route.as_ref() {
            body["model"] = Value::String(route.upstream_model.clone());
            log::info!(
                "[Qoder] resolved local route {} -> provider={}, model={}",
                requested_model,
                route.provider_id,
    ...
    let mut ctx = RequestContext::new_with_provider_id(
        ...
        tag,
        app_type_str,
        qoder_route.as_ref().map(|route| route.provider_id.as_str()),
    )
    .await?;
...
fn build_chat_proxy_error_response(
    ...
    let body = chat_proxy_error_json(
        ctx.tag,
        &ctx.provider.name,
        &ctx.request_model,
        endpoint,
...
/// 兼容 Responses handler 的旧函数名；Responses 仍然只由 Codex 使用。
fn build_codex_proxy_error_response(
    ctx: &RequestContext,
    endpoint: &str,
    error: &ProxyError,
) -> Result<axum::response::Response, ProxyError> {
    build_chat_proxy_error_response(ctx, endpoint, error)
}

fn chat_proxy_error_json(
    tag: &str,
    provider_name: &str,
    request_model: &str,
    endpoint: &str,
    ...
    let message = if upstream_status == Some(413) {
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "CC Switch local proxy failed while handling {tag} endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
/// 兼容既有测试使用的 Codex 错误格式入口。
///
/// 生产代码已统一走 [`chat_proxy_error_json`]，保留此函数仅为确保原有
/// Responses 错误体契约继续受测试覆盖。
#[cfg(test)]
fn codex_proxy_error_json(
    ...
    chat_proxy_error_json("Codex", provider_name, request_model, endpoint, error)
}
