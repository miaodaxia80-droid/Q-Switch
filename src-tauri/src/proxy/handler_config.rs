...
/// OpenAI Chat Completions API 解析配置（用于 Qoder /qoder/v1/chat/completions）
pub const QODER_PARSER_CONFIG: UsageParserConfig = UsageParserConfig {
    stream_parser: TokenUsage::from_openai_stream_events,
    response_parser: TokenUsage::from_openai_response,
    model_extractor: openai_model_extractor,
    stream_event_filter: Some(openai_stream_usage_event_filter),
...
/// Qoder Chat Completions Handler 配置
#[allow(dead_code)]
pub const QODER_CHAT_HANDLER_CONFIG: HandlerConfig = HandlerConfig {
    app_type: AppType::Qoder,
    tag: "Qoder",
    app_type_str: "qoder",
    parser_config: &QODER_PARSER_CONFIG,
};
